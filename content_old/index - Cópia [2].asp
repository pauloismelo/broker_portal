...
<html lang="pt-BR">
	...
	boxop=split(operadorax,",")
	for w=0 to ubound(boxop)
	response.Write("select * from INFORMATIVOS_DEMAN where CHARINDEX('"&boxop(w)&"', operadoras)>0 order by id desc ")
	set ppo=conexao.execute("select * from INFORMATIVOS_DEMAN where CHARINDEX('"&boxop(w)&"', operadoras)>0 order by id desc ")
	
	'set ppo=conexao.execute("select * from STB_ARTIGOS where categoria='POPUPCLIENTE' order by id desc")
	if not ppo.eof then
	%>
        ...
        <div class="modal-content">
            ...
            <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
              ...
              <div class="carousel-inner">
                <%set ppo2=conexao.execute("select * from INFORMATIVOS_DEMAN where CHARINDEX('"&boxop(w)&"', operadoras)>0 order by id desc ")
				y=0
				while not ppo2.eof
					if ppo2("foto")<>"" then%>
